...
from import_english_preview import GROUPS, parse_answer_keys, load_legacy_parser, normalise_metadata, split_option_support_boundary
...
def parse_support_blocks(raw_text: str) -> dict | None:
...
print("Testing full simulation on EFOMM...")
...
supp = parse_support_blocks(initial)
print(json.dumps(supp, indent=2, ensure_ascii=False))

print("\nTesting full simulation on CN Q1 & Q2...")
cn_page_text = reader.pages[11].extract_text()
cn_q1_raw = cn_page_text[list(re.finditer(r"(?m)^\s*1\s*\)\s*", cn_page_text))[0].end():list(re.finditer(r"(?m)^\s*2\s*\)\s*", cn_page_text))[0].start()]
print("CN Q1 raw statement formatted:")
print(format_statement_items(clean_ocr(cn_q1_raw)))
