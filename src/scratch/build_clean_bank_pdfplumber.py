...
from split_perfect import split_reading_statement_perfect, clean_subject_tags_from_header
...
def format_reading_text(raw_text):
    ...
    raw_text = clean_subject_tags_from_header(raw_text)
    raw_text = re.sub(r'^(?:Texto\s+(?:para\s+a\s+questão|[IVX\d]+)|Texto\s*:\s*)\s*\n*', '', raw_text, flags=re.IGNORECASE).strip()
    raw_text = clean_subject_tags_from_header(raw_text)
    ...
    for line in lines:
        is_new_p = (
            re.match(r'^(?:[0-9]+[º°§]|§\s*[0-9]+|\([0-9]+\)|[IVXLCDM]+\b|—|\–|\-|\"|\“|Texto\s+[IVX\d]+|HOMEM NO MAR|A LIBERDADE|Cultura|Sobre a|Noruega|Rússia|Como ser|A sociedade|Felicidade|A solidão|A Última Crônica|O bobo|Folia|Mulher na Marinha|O Médico|Quando a comida|Linguagem inclusiva|A comunicação inclusiva)', line, re.IGNORECASE) or
...
for cfg in PDF_CONFIGS:
    ...
    print(f"\nProcessing {cfg['file']} with pdfplumber...")
    full_text = ""
    ...
    for i, m in enumerate(matches):
        ...
        formatted_statement = clean_subject_tags_from_header(formatted_statement)
